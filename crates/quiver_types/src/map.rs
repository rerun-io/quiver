...
impl<K: LogicalType + 'static, V: LogicalType + 'static> LogicalType for Map<K, V> {
    ...
    fn slice_typed(typed: &Self::Typed, offset: usize, length: usize) -> Option<Self::Typed> {
        // Slicing only moves the parent's window: the entries are shared whole
        // and the offsets stay absolute, so both children's views carry over.
        Some(TypedMap {
            map: typed.map.slice(offset, length),
            keys: typed.keys.clone(),
