    ...
    ($marker:ident, $typed:ident, $array:ty, $variant:ident) => {
        ...
        impl<L: LogicalType + 'static> LogicalType for $marker<L> {
            ...
            unsafe fn value_unchecked(typed: &Self::Typed, index: usize) -> Self::Value<'_> {
                ...
                // entries, so `index` and `index + 1` are both valid positions.
                let (start, end) = unsafe {
                    (
                        offsets.get_unchecked(index).as_usize(),
                        offsets.get_unchecked(index + 1).as_usize(),
                    )
                };
                ListValue::new(&typed.values, start, end)
            }
